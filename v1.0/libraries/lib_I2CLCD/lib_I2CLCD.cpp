...
void createCharLCD(uint8_t slot, uint8_t bitmap[8]) {
    // send CGRAM address command
    Wire.beginTransmission(addr);
    Wire.write(0x80);                  // command mode (as used in the library)
    Wire.write(0x40 | (slot << 3));    // CGRAM address: slot 0=0x40, 1=0x48, 2=0x50...
    Wire.endTransmission();
    delayMicroseconds(40);

    // send the 8 pixel rows
    Wire.beginTransmission(addr);
    ...
    // return to DDRAM so normal printing still works
    returnHomeLCD();
}

void printCustomCharLCD(uint8_t slot) {
    Wire.beginTransmission(addr);
    Wire.write(0x40);                  // data mode
    Wire.write(slot);                  // slot 0-7
    Wire.endTransmission();
}